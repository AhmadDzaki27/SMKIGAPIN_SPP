...
<footer class="main-footer">
    <strong>Copyright &copy; 2023 <a href="http://adminlte.io">SMKS IGASAR PINDAD </a>.</strong>
    All rights reserved. | Repost by <a href='https://www.linkedin.com/in/ahmad-dzaki-9394bb261' title='Ahmad Dzaki' target='_blank'>Ahmad Dzaki</a>
...
<script>
    ...
    $(document).ready(function() {
        $('#SiswaBelumLunas').change(function() {
            ...
            $.ajax({
                type: 'post',
                url: '<?= base_url() ?>DataPembayaranSPP/detail_siswa/true',
                data: {
                    "nisn": nisn
                },
        ...
        $('#selectjurusan').change(function() {
            ...
            $.ajax({
                type: 'post',
                url: "<?= base_url('DataSPPSiswa/cariKelas') ?>",
                data: {
        ...
        $('#detailDPP').on('show.bs.modal', function(event) {
            ...
            $.ajax({
                type: 'post',
                url: '<?= base_url() ?>DataSPPSiswa/tampildata',
                data: {
                    'nisn': nisn
                },
                success: function(data) {
                    var dataSPPSiswa = JSON.parse(data);
                    modal.find('#nisn').text(dataSPPSiswa.nisn)
                    modal.find('#nominal_dpp').text(dataSPPSiswa.nominal_dpp)
                    modal.find('#jumlah_angsuran').text(dataSPPSiswa.jumlah_angsuran)
                    modal.find('#nominal_angsuran').text(dataSPPSiswa.nominal_angsuran)
                    var status = "";
                    if (dataSPPSiswa.status == 0) {
                        status = "belum lunas"

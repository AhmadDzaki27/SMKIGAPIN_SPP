                        ...
                        <form class="form" action="<?= base_url('DataPembayaranUjian/tambahData/') . $nisn ?>" method="POST">
                            <div class="form-group">
                                ...
                                <select class="form-control" name="tahunAjaran" id="tahunAjaran">
                                    ...
                                    $index = null;
                                    for ($i = 1; $i <= 3; $i++) {
                                        if ($siswa['tbl_kelas' . $i] !== null) {
                                            if ($index === null) {
                                                $index = 0;
                                            }
                                    ?>
                                            <option data-tahun="<?= $tahunAjaran[$index]->kode_ta ?>" value="<?= $tahunAjaran[$index]->kode_ta . '-' . $siswa['tbl_kelas' . $i] ?>" data-kelas='<?= $siswa['tbl_kelas' . $i] ?>'><?= $tahunAjaran[$index]->tahun_ajaran . '::' . $siswa['tbl_kelas' . $i] ?></option>
                                    <?php
                                            $index++;
                                        }
                                    }
                                    ...
                                    ?>
                                    <!-- <option data-tahun="<?= $valueTahunAjaran->kode_ta ?>" value="<?= $valueTahunAJaran->kode_ta . '-' . $siswa['tbl_kelas' . $no] ?>" data-kelas='<?= $siswa['tbl_kelas' . $no] ?>'><?= $valueTahunAJaran->tahun_ajaran . '::' . $siswa['tbl_kelas' . $no] ?></option> -->
                                    <?php
                                    //     }
                                    //     $no++;
                                    // endforeach; 
